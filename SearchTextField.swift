...
open class SearchTextField: UITextField {
    ...
    /// Set an array of strings to be used for suggestions
    open func addCityStr( city : String) {
        filterDataSource.append( SearchTextFieldItem(title: city) )
    }
    
    /// Delete an array of strings to be used for suggestions
    open func delCityStr() {
    ...
    open var highlightAttributes: [NSAttributedStringKey: AnyObject] = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue):UIFont.boldSystemFont(ofSize: 12)]
...
/*    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        // Creates an action that fetches location names from the network
                    ...
                    for iRes in result
                    {
                        let item = SearchTextFieldItem( title: cities )
...
*/
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        var searchtxt = text!
            searchtxt = searchtxt.uppercased()

        var cityStr = [String]()
        // If text's charater check
        if searchtxt[searchtxt.index(searchtxt.startIndex, offsetBy: 0 )] >= "A" &&
            searchtxt[searchtxt.index(searchtxt.startIndex, offsetBy: 0 )] <= "Z" {

            let firstStr = String(searchtxt[searchtxt.index(searchtxt.startIndex, offsetBy: 0 )]) + "_cityname"
            if let path = Bundle.main.path(forResource: firstStr, ofType: "txt") {
                do {
        ...
        for i in 0 ..< cityStr.count {
            
            let item = SearchTextFieldItem(title: cityStr[i])
...
public struct SearchTextFieldTheme {
    ...
    public static func lightTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor.clear, borderColor: UIColor.clear, separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 14), fontColor: UIColor.black)
    }
    
    public static func darkTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 0.8, green: 0.8, blue: 0.8, alpha: 0.6), borderColor: UIColor (red: 0.7, green: 0.7, blue: 0.7, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 14), fontColor: UIColor.white)
    }
}
